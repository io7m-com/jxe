 ...
 *
 * A schema definition is typically used with a {@link JXEHardenedDispatchingResolver} to
 * resolve schemas from internal Java resources. When an XML parser is required to
 * locate the XSD schema for a namespace {@code n}, it consults the map of
 * schema definitions and, if a schema exists with the given namespace URI,
 * passes the {@link #fileIdentifier()} to the dispatching resolver which then
